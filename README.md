...
A scientific Python project building a validated adaptive mesh refinement (AMR) framework from a conservative uniform-grid foundation. The current `v0.2` stage adds the basic one-dimensional hierarchy and conservative level-transfer operations while retaining the verified uniform advection baseline.
...
Basic AMR data structures and transfer operations are now implemented and independently tested. A static hierarchy can be generated directly from gradient flags; it is not yet advanced in time.

![Static gradient-selected AMR hierarchy](figures/phase2_static_hierarchy.png)
